...
export async function initializeDatabase() {  
  ...
  if (!db) {
    ...
    // Create webhook_outcomes table for tracking webhook processing results
    await db.exec(`
    ...
    // Create index for faster webhook outcomes lookups
    await db.exec(`
      CREATE INDEX IF NOT EXISTS idx_webhook_outcomes_lookup 
...
export async function upsertOrder(orderData: any) {
  ...
  const sql = `
    ...
    ON CONFLICT(order_id) DO UPDATE SET
      ...
      -- Preserve market data captured at trade execution
      bid_at_open = COALESCE(bid_at_open, excluded.bid_at_open),
      ask_at_open = COALESCE(ask_at_open, excluded.ask_at_open),
      spread_at_open = COALESCE(spread_at_open, excluded.spread_at_open),
      consider_ob_reference = COALESCE(consider_ob_reference, excluded.consider_ob_reference),
      ...
      -- Preserve TradingView-specific parameters - only update if existing value is NULL
      alert_id = COALESCE(alert_id, excluded.alert_id),
      maxobalert = COALESCE(maxobalert, excluded.maxobalert),
      diff_op_ob = COALESCE(diff_op_ob, excluded.diff_op_ob),
      timeframe = COALESCE(timeframe, excluded.timeframe),
      exchange = COALESCE(exchange, excluded.exchange),
      findObType = COALESCE(findObType, excluded.findObType),
      filterFvgs = COALESCE(filterFvgs, excluded.filterFvgs),
      fvgDistance = COALESCE(fvgDistance, excluded.fvgDistance),
      lineHeight = COALESCE(lineHeight, excluded.lineHeight),
      filterFractal = COALESCE(filterFractal, excluded.filterFractal)
  `;
