...
export async function getAccessToken(useSecondaryApi = false): Promise<string> {
  ...
  // Secondary API is disabled - all accounts use primary API
  if (useSecondaryApi) {
    console.warn('Secondary API requested but is disabled, using primary API instead');
    // Fall through to use primary API
  }
...
export async function getActiveOrders(
  ...
  try {
    // Force using secondary API for account 3979937 - DISABLED
    // const shouldUseSecondaryApi = useSecondaryApi || loginNumber === "3979937";
    const shouldUseSecondaryApi = useSecondaryApi;
...
export async function placeTrade(
  ...
  // const shouldUseSecondaryApi = useSecondaryApi || loginNumber === "3979937"; - DISABLED
  const shouldUseSecondaryApi = useSecondaryApi;
