...
import { config, DEFAULT_ACCOUNT_NUMBER, SECONDARY_API_ACCOUNTS, ALL_MONITORED_ACCOUNTS } from "./config"
...
/**
 * Places a trade with retry logic and spread validation
 ...
 * 
 * Features:
 * - Validates spread before placing trade based on market conditions
 * - Retries up to 5 times if spread is too high
 * - Uses dynamic spread limits: normal/news/overnight/lowVolatility
 * - Forex: 8-25 pips, Indices: 150-500 points depending on conditions
 */
export async function placeTradeWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      // First, wait for acceptable spread
      const spreadCheck = await waitForAcceptableSpread(symbol);
      if (!spreadCheck.success) {
        throw new Error(`Spread validation failed after ${spreadCheck.attempts} attempts: ${spreadCheck.error}`);
      }
      ...
      // Final spread validation with current market data
      const currentSpread = marketData.ask - marketData.bid;
      const finalSpreadValidation = validateSpread(symbol, currentSpread);
      if (!finalSpreadValidation.valid) {
        throw new Error(`Final spread check failed: ${finalSpreadValidation.error}`);
      }

      // Calculate entry price
      const entryPrice = (side === "BUY" || side === "B") ? marketData.ask : marketData.bid;

      // Log market data and calculation inputs with spread info
      const specs = getInstrumentSpecs(symbol);
      const spreadInPips = currentSpread / specs.pipValue;
      const spreadUnit = specs.type === 'index' ? 'pts' : 'pips';
      const marketCondition = getCurrentMarketCondition();

      const marketLog = `MARKET_DATA: ${symbol} | Bid: ${marketData.bid} | Ask: ${marketData.ask} | Spread: ${spreadInPips.toFixed(1)}${spreadUnit} | Entry: ${entryPrice} | Side: ${side} | TP_Pips: ${takeProfitPips} | SL_Pips: ${stopLossPips} | Condition: ${marketCondition} | Account: ${loginNumber}`;
 ...
 */
function getCurrentMarketCondition(): 'normal' | 'news' | 'overnight' | 'lowVolatility' {
  const now = new Date();
  ...
  if ((estHour === 8 && utcMinute >= 15 && utcMinute <= 45) ||
...
/**
 * Waits for acceptable spread with retry logic
 */
async function waitForAcceptableSpread(
  symbol: string,
  maxRetries: number = config.SPREAD_VALIDATION.maxRetries
): Promise<{ success: boolean; spread?: number; error?: string; attempts: number }> {
  let attempts = 0;
  let lastError = '';
  ...
  while (attempts < maxRetries) {
    ...
    try {
      ...
      if (marketData.isStale) {
        lastError = `Market data too old for ${symbol}`;
        if (attempts < maxRetries) {
          const delay = config.SPREAD_VALIDATION.retryDelayMs * Math.pow(config.SPREAD_VALIDATION.backoffMultiplier, attempts - 1);
          await setTimeoutPromise(delay);
          continue;
        }
        break;
      ...
      const currentSpread = marketData.ask - marketData.bid;
      const validation = validateSpread(symbol, currentSpread);

      if (validation.valid) {
        logTrade.spread(symbol, 'ACCEPTED', currentSpread, validation.maxAllowed, validation.condition, attempts);
        return {
          success: true,
      ...
      lastError = validation.error || 'Spread validation failed';

      // Log spread rejection with retry info
      logTrade.spread(symbol, 'REJECTED', currentSpread, validation.maxAllowed, validation.condition, attempts, maxRetries);
      ...
      lastError = `Error checking spread: ${error.message}`;
      if (attempts < maxRetries) {
        const delay = config.SPREAD_VALIDATION.retryDelayMs * Math.pow(config.SPREAD_VALIDATION.backoffMultiplier, attempts - 1);
        await setTimeoutPromise(delay);
      }
...
app.get("/api/test-keys", async (req, res) => {
  try {
    ...
    // Test secondary API key - DISABLED
    let secondaryResult: any = { success: false, error: "Secondary API disabled" }
    /*
    try {
  ...
  try {
    ...
    while (hasMoreOrders) {
      try {
        ...
        logger.error(`Error fetching page ${page} of closed orders: ${error instanceof Error ? error.message : String(error)}`)
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      logger.error(`WEBHOOK ERROR: Invalid JSON payload: ${error instanceof Error ? error.message : String(error)}`);
    ...
    logger.error(`Error queueing webhook: ${error instanceof Error ? error.message : String(error)}`);
    res.status(500).json({
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const lastQuote = await webSocketManager.getQuoteForSymbol(symbol);
        if (!lastQuote) {
    ...
    logger.error(`Webhook processing failed for ${symbol} after ${executionTime}ms: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
}
...
app.get("/api/status/:loginNumber?", async (req, res) => {
  try {
    ...
    // Use the primary default account number (secondary API disabled)
    const defaultAccount = DEFAULT_ACCOUNT_NUMBER;
    ...
    try {
      const lastQuote = await webSocketManager.getQuoteForSymbol("EURUSD");
      ...
      if (apiError.response?.data?.code === 1405 && !useSecondaryApi) {
        ...
        try {
          ...
          logger.error(`Retry with secondary API also failed for account ${loginNumber}: ${retryError instanceof Error ? retryError.message : String(retryError)}`);
          throw retryError;
        }
      }
    ...
    logger.error(`Error fetching status: ${error instanceof Error ? error.message : String(error)}`);
...
app.get("/api/db-orders/:loginNumber", SecondaryAuth, async (req, res) => {
  try {
    ...
    logger.error(`Error fetching database orders: ${error instanceof Error ? error.message : String(error)}`)
...
app.get("/api/recent-db-orders/:loginNumber", SecondaryAuth, async (req, res) => {
  try {
    ...
    logger.error(`Error fetching recent database orders: ${error instanceof Error ? error.message : String(error)}`)
...
app.get("/api/webhook-outcomes/:loginNumber", SecondaryAuth, async (req, res) => {
  try {
    ...
    logger.error(`Error fetching webhook outcomes: ${error instanceof Error ? error.message : String(error)}`)
...
app.get("/api/deposits/:loginNumber", SecondaryAuth, async (req, res) => {
  try {
    ...
    logger.error(`Error fetching deposits: ${error instanceof Error ? error.message : String(error)}`)
    res.status(500).json({ error: "Internal server error" })
  }
})
...
// Test endpoint for spread validation
app.get("/api/test-spread/:symbol", SecondaryAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      currentSpread: {
        raw: currentSpread,
        inPips: Number(spreadInPips.toFixed(1)),
        unit
      },
      validation: {
      ...
      spreadLimits: specs.type === 'index' ? config.SPREAD_LIMITS.indices : config.SPREAD_LIMITS.forex,
      timestamp: new Date().toISOString()
    });
  } catch (error: any) {
    res.status(500).json({
...
// Test endpoint for spread waiting functionality
app.get("/api/test-spread-wait/:symbol", SecondaryAuth, async (req, res) => {
  try {
    const symbol = req.params.symbol.toUpperCase();
    const maxRetries = parseInt(req.query.maxRetries as string) || 3;

    const startTime = Date.now();
    const result = await waitForAcceptableSpread(symbol, maxRetries);
...
app.get("/api/test-order-counts/:loginNumber", SecondaryAuth, async (req, res) => {
  try {
    ...
    logger.error(`Error fetching order counts: ${error instanceof Error ? error.message : String(error)}`)
    res.status(500).json({
...
app.get("/api/chart-data", SecondaryAuth, async (req, res) => {
  try {
    ...
    const currentQuote = await webSocketManager.getQuoteForSymbol(symbol);
...
// Cron job to update all orders - configurable interval
if (config.CRON.ENABLE_ORDER_SYNC && config.CRON.ORDER_SYNC_INTERVAL > 0) {
  const cronSchedule = `*/${config.CRON.ORDER_SYNC_INTERVAL} * * * *`;
  logger.info(`Order sync cron enabled: running every ${config.CRON.ORDER_SYNC_INTERVAL} minutes`);
    ...
    for (const loginNumber of loginNumbers) {
      ...
      for (const apiOrder of allApiOrders) {
        ...
        await upsertOrder({
          ...
          realSlPips: realSlPips || (dbOrder ? dbOrder.real_sl_pips : null),
          realTpPips: realTpPips || (dbOrder ? dbOrder.real_tp_pips : null),
          spreadAtOpen: null, // Preserve existing spread data
          considerObReference: null, // Preserve existing value
          bidAtOpen: null, // Preserve existing bid data
          askAtOpen: null, // Preserve existing ask data
          closeTime: apiOrder.closeTime,
          durationInMinutes: durationInMinutes,
          // Preserve TradingView parameters - pass null to let database preserve existing values
          alertId: null,
          alertThreshold: null,
          maxobalert: null,
          max_size: null,
  ...
  logger.info('Order sync cron disabled');
}

// Weekly log cleanup cron job - REMOVED (not needed for lightweight setup)
// Log cleanup should be handled manually or via system-level log rotation
...
async function syncMissingOrder(order: any, retryCount = 0) {
  ...
  try {
    ...
    const orderData = {
      ...
      // For missing orders, preserve TradingView parameters by passing null
      // The database will keep existing values if they exist
      maxobalert: null,
      filterFractal: null,
      findObType: null,
      fvgDistance: null,
    ...
    if (errorMessage.includes('no such column') || errorMessage.includes('has no column named')) {
      ...
      try {
        ...
        logger.error(`Failed to run database migration for order ${order.id}: ${migrationError instanceof Error ? migrationError.message : String(migrationError)}`)
      }
    }
    ...
    logger.error(`Error syncing missing order ${order.id} to database after ${retryCount} retries: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
httpsServer
  ...
  .on("error", (e: NodeJS.ErrnoException) => {
    logger.error(`Failed to start HTTPS server on port ${port}: ${e instanceof Error ? e.message : String(e)}`)
