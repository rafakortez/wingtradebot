...
class WebhookQueue extends EventEmitter {
  ...
  private async loadProcessedIdsFromDatabase(): Promise<void> {
    try {
      ...
      logger.error(`Failed to load processed IDs from database: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async add(data: any, accountNumber: string): Promise<string> {
    ...
    logger.debug(`Webhook queued: ${jobId} | queueLength: ${this.queue.length} | account: ${accountNumber}`);
  ...
  async storeProcessedId(alertId: string, accountNumber: string): Promise<void> {
    try {
      ...
      logger.error(`Failed to store processed ID ${alertId} for account ${accountNumber}: ${error instanceof Error ? error.message : String(error)}`);
  ...
  private async cleanupOldProcessedIds(): Promise<void> {
    try {
      ...
      logger.error(`Failed to cleanup old processed IDs: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async processQueue() {
    ...
    while (this.queue.length > 0) {
      ...
      try {
        ...
        logger.error(`Failed to process webhook job: ${job.id} | error: ${error.message} | retries: ${job.retries}`);
  ...
  private async cleanupProcessedIds() {
    try {
      ...
      logger.error(`Failed to cleanup processed IDs: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
